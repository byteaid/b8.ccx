...
- `docs/REQUIREMENT.md`, `docs/GLOSSARY.md`, `docs/DATA-MODEL.md`, `docs/features/**` (minus `dataflows/`) — analyst.
- `docs/SOLUTION.md`, `docs/features/FT-*/dataflows/DF-*.md`, `${OS_TEMP}/aix-todo/{repo-basename}/todo.md` — architect.
...
|---|---|---|
| `code-maps-to-dataflow` | The implementation maps step-by-step to the `DF-NNN` data flows the slice was briefed with — every documented step is locatable in the changed code, in order, on the named infrastructure; every meaningful data transformation in the changed code traces back to a documented step. Severity `blocker` either way: extra/missing/re-ordered pipeline work is either wrong code (developer corrects) or a stale data flow (architect rewrites) — the orchestrator routes, the slice does not close unresolved. See § "Code↔data-flow mapping check". | `development-documentation` § data-flow |
...
1. **Snapshot the slice.** Re-read the developer's hand-off and the orchestrator's brief. Note: files changed, the test FQN(s), the `DF-NNN` data flows the developer was briefed with, the originating `BL-NNN` / `BG-NNN`.
...
6. **Cross-link.** Each finding may link to:
   ...
   - The data flow it relates to (`DF-NNN`) — mandatory for `code-maps-to-dataflow` findings.
...
### Code↔data-flow mapping check (`code-maps-to-dataflow`)

This check is non-grep — it is a reading pass against the architect's pipeline contract:

1. Collect the `DF-NNN` files from the orchestrator's brief. If the brief names none AND the slice touched a documented flow (its `FL-NNN` resolves under `docs/features/`), check the owning feature's `dataflows/` folder yourself; a documented flow implemented without any data flow is itself a `code-maps-to-dataflow` finding (severity `blocker` — the architect's pass was skipped).
2. For each `DF-NNN`, walk its Steps table against the changed code: every step must be locatable (same order, same specific infrastructure — the step says "Azure SQL table `Users`", the code must hit that table, not a cache).
3. Walk the reverse direction: every meaningful data transformation, persistence call, or message emission in the changed code must trace back to a step (or to another `DF-NNN` of the same flow).
4. Check the Terminal state and Error paths sections — side effects the code produces but the DF does not declare (or vice versa) are findings.
5. Classify: code deviates from a correct DF → `blocker`, corrective owner `dotnet-developer`. Code is right but the DF is stale → `blocker`, corrective owner `dotnet-architect` (DF rewrite), noted explicitly in the recommendation. Style differences (class shapes, idioms, naming) are NEVER findings under this rule — the DF constrains the pipeline, not the style.
...
- `development-documentation` § data-flow — the `DF-NNN` shape the `code-maps-to-dataflow` rule maps code against.
- `development-documentation` § id-taxonomy — the `DT-NNN` prefix.
- `dotnet-conventions` § forbidden-patterns — the primary rule catalog. Every slug you write into `debt.md` lives in its cited skill leaf: most in `dotnet-conventions`, the testing-conformance slugs in `dotnet-testing`, and `code-maps-to-dataflow` in `development-documentation` § data-flow (see the Skill leaf column of § "Rule set you check").
