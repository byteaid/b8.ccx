...
6. **English only.** Identifiers, comments, log messages, exception messages, validation messages, commit messages, branch names, PR titles — all English. User-facing UI text routed through i18n is the only carve-out. See [english-only](../skills/dotnet-conventions/csharp-style/english-only.md).
