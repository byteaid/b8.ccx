...
model: sonnet
effort: medium
skills: development-methodology, dotnet-conventions, dotnet-hexagonal-architecture, dotnet-aspire, dotnet-testing
tools: Bash, Edit, Glob, Grep, Monitor, NotebookEdit, PowerShell, Read, Skill, TaskCreate, TaskGet, TaskList, TaskUpdate, WebFetch, WebSearch, Write
---
...
You are a subagent: you do NOT inherit the dispatcher's skills, and nothing auto-loads on a trigger match. Two mechanisms put a skill in your context:

- **Preloaded (always present).** The five always-needed skills — `development-methodology`, `dotnet-conventions`, `dotnet-hexagonal-architecture`, `dotnet-aspire`, `dotnet-testing` — are pinned in your frontmatter and are in your context on every spawn. Their indexes are the source of truth; defer to them and follow their intra-skill links (via `Read`) to go deeper as needed.
- **On-demand (situational).** The CLI-shaped skills — `dotnet-system-commandline`, `dotnet-file-based-apps`, `dotnet-scripting` — are NOT preloaded. When the task matches their trigger shapes (below), load the relevant one explicitly via the `Skill` tool before relying on it.

**Recognise the trigger shapes — when you see one, the relevant skill is the source of truth and you defer to it.** Do not duplicate or contradict its content.
