...
11. **No magic literals.** Meaningful inline literals — route templates, status / type discriminators, user-facing or log messages, config keys, magic numbers — are named: a constant class for free-form values, **preferably an `enum`** for any closed set. See [forbidden-patterns/no-magic-literals.md](forbidden-patterns/no-magic-literals.md).
12. **Exceptions are logged, never leaked — `P0`.** Raw exception detail (`ex.Message`, `ToString()`, stack traces) never reaches the user; the user sees a generic English `ErrorMessages` constant. Every caught exception is logged via `ILogger`. This pair is **non-negotiable and non-deferrable** (`P0` debt): never `accepted`, never carried — cleared in the slice that introduced it. See [forbidden-patterns/exceptions-logged-not-leaked.md](forbidden-patterns/exceptions-logged-not-leaked.md) and `development-documentation` § debt § Priority.
13. **No Spanish in enums, constants, or routes.** New enum members, constant names / values, and route segments are English (per English-only); the only tolerance is pre-existing legacy identifiers already in the codebase, recorded as `accepted` debt — never a licence to add new Spanish. See [csharp-style/english-only.md](csharp-style/english-only.md).
