...
4. **Per-class AppHost mount.** `[ClassInitialize]` builds the `DistributedApplication`; `[ClassCleanup]` disposes it. **No shared `AppHostFixtureBase`, no `[AssemblyInitialize]`.** State from class A cannot leak into class B by construction. See [mstest-integration.md](mstest-integration.md).
...
12. **Logs and artefacts land under `TestResults/{run-id}/...`.** The fixture reads `BLAZTRAP_TEST_RUN_DIR` first, falls back to `TestContext.TestRunDirectory`, then to `AppContext.BaseDirectory`. Per-resource logs come from `Blaztrap.Aspire.FileLogging` (`appHost.AddFileLogging(logsDir)` after every `AddProject`/`AddExecutable` and before `Build()`). See [mstest-integration.md](mstest-integration.md) § Test artefacts.
