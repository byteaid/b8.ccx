...
Stage 1 of the usage-report pipeline. Uses `Azure.ResourceManager.ArmClient` with `DefaultAzureCredential` to list every subscription the operator can read, optionally filtered by a regex. Output feeds stage 2 (`discover-resources`) when no `--subscription` argument is passed there.
